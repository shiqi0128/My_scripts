...
# b、用例所在的序列类型
    ...
    def test_register(self, one_testcase):
        ...
        # type(res.text)
        try:
            self.assertEqual(one_testcase["expected_value"], real_code, one_testcase["name"])
        except AssertionError as e:
            do_log.error(f"{one_testcase}:具体异常为{e}")
            do_excel.write_data(one_testcase, do_yaml.get_data("excel", "result_column"), "失败")
            raise e
        else:
            do_log.debug(res.text)
            do_excel.write_data(one_testcase, do_yaml.get_data("excel", "result_column"), "成功")
"""
