"""此文件为gmall的PC端巡店逻辑所有 接口测试被调方法文件"""
import unittest
import requests
import json
import random
from time import sleep
from interfaceTest.GMALL.gmall_all.Intelligent_patrol.patrol_header import get_header_pc, host
...
2. 当添加巡查项和巡查问题时，parent_id 为要添加的巡查大类id，children不能为空，格式看“请求参数”
        """
    url = host + "/api/company/patrol-issues"
    data = {"parent_id": 0, "content": "鸡脚旮旯%s" % random.randrange(1, 1000)}       # 创建巡查大类
...
parent_id, problem_id = Problem_creat()


def Patrol_add(start_date, end_date):
    """巡查计划--新增
...
scheduleId = Patrol_add("2019-04-01", "2019-06-30")
...
3. 若children中只有一个字段content，代表此数据为新增，若存在id，代表修改该id对应问题
    ...
    def Problem_delete(self):
        """问题分类--删除"""
        list_p = [problem_id, parent_id]        # 必须先删除巡查项和问题才能删除巡查大类
        print(list_p)
        for p in list_p:
    ...
    def Patrol_detail(self):
        ...
        """
        url = host + "/api/company/patrol-schedules/%s" % scheduleId
        i = 1
        while i <= 5:
            try:
                r = requests.get(url=url, headers=header).json()
                ...
                url_01 = host + "/api/company/patrol-schedules/%s" % scheduleId
                r = requests.patch(url=url_01, headers=header).json()
                out_format("巡查计划 - 修改状态", r)
            i +=1

    def Patrol_edit(self, start_date, end_date, scheduleId):
        """巡查计划--修改
        :param:start_date:任务开始时间
        :param:end_date:任务结束时间
        :param:name:任务名称
