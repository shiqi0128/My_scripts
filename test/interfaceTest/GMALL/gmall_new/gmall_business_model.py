...
def Merch_sales(start_date, end_date, shop_id=golbal_shopid):
    ...
    :param:sort_field:order_num : 订单量; cusomer_num ：下单人数;sales_num ：商品销量; sales_amount：商品金额 unit_price：销售单价--非必填
    ...
    if r["data"]["stats"]["data"] is not None:
        ...
        for i in range(s):
            ...
            print("产品id列表:", product_ids)
            for index, product_id in enumerate(product_ids, 1):
                print("获取产品id[%s]:" % index, product_id)
...
product_id = Merch_sales("2019-03-01", "2019-03-01")
